...
void button::loop()
{
  ...
  if (millis() - this->current_Millis > 100)
  {
    if (this->count_button == 0 && digitalRead(this->button_pin)) // 1Click nút 'Mode' + đè nút
    {
      ...
      debug("1 click hold");
      // ---
      if (button_1ClickHold)
        button_1ClickHold();     // Tức con trỏ hàm button_1ClickHold phải có giá trị địa chỉ khác 0 (tránh RESET), thì mới được dùng button_1ClickHold();
